...
void maybe_drop_secondary_weapon_egg(object *player, int weapon_flag, int powerup_num, int count)
{
	if (Players[player->id].secondary_weapon_flags & weapon_flag) {
		...
		for (i=0; i<max_count; i++)
			call_object_create_egg(player, 1, OBJ_POWERUP, powerup_num);
	}
}
...
//	mprintf((0, "In drop_player_eggs...\n"));
	...
	if ((player->type == OBJ_PLAYER) || (player->type == OBJ_GHOST)) {
		...
		if (Players[pnum].flags & PLAYER_FLAGS_CLOAKED)
			call_object_create_egg(player, 1, OBJ_POWERUP, POW_CLOAK);

		//	Drop the primary weapons
		objnum = maybe_drop_primary_weapon_egg(player, HAS_VULCAN_FLAG, POW_VULCAN_WEAPON);
		if (objnum!=-1)
			Objects[objnum].ctype.powerup_info.count = Players[pnum].primary_ammo[VULCAN_INDEX];

		maybe_drop_primary_weapon_egg(player, HAS_SPREADFIRE_FLAG, POW_SPREADFIRE_WEAPON);
		maybe_drop_primary_weapon_egg(player, HAS_PLASMA_FLAG, POW_PLASMA_WEAPON);
		maybe_drop_primary_weapon_egg(player, HAS_FUSION_FLAG, POW_FUSION_WEAPON);
		...
		maybe_drop_secondary_weapon_egg(player, HAS_PROXIMITY_FLAG, POW_PROXIMITY_WEAPON, (Players[player->id].secondary_ammo[PROXIMITY_INDEX]+2)/4);
		maybe_drop_secondary_weapon_egg(player, HAS_SMART_FLAG, POW_SMARTBOMB_WEAPON, Players[player->id].secondary_ammo[SMART_INDEX]);
		maybe_drop_secondary_weapon_egg(player, HAS_MEGA_FLAG, POW_MEGA_WEAPON, Players[player->id].secondary_ammo[MEGA_INDEX]);

		num_missiles = Players[pnum].secondary_ammo[HOMING_INDEX];
		if (num_missiles > 6)
			num_missiles = 6;
		call_object_create_egg(player, num_missiles/4, OBJ_POWERUP, POW_HOMING_AMMO_4);
		call_object_create_egg(player, num_missiles%4, OBJ_POWERUP, POW_HOMING_AMMO_1);

		//	If player has vulcan ammo, but no vulcan cannon, drop the ammo.
		if (!(Players[player->id].primary_weapon_flags & HAS_VULCAN_FLAG)) {
			int	amount = Players[player->id].primary_ammo[VULCAN_INDEX];
			if (amount > 200) {
				mprintf((0, "Surprising amount of vulcan ammo: %i bullets.\n", amount));
				amount = 200;
			}
			while (amount > 0) {
				call_object_create_egg(player, 1, OBJ_POWERUP, POW_VULCAN_AMMO);
				amount -= VULCAN_AMMO_AMOUNT;
			}
		}

		//	Drop the player's missiles.
		num_missiles = Players[pnum].secondary_ammo[CONCUSSION_INDEX];
		if (num_missiles > 4)
			num_missiles = 4;

		call_object_create_egg(player, num_missiles/4, OBJ_POWERUP, POW_MISSILE_4);
		call_object_create_egg(player, num_missiles%4, OBJ_POWERUP, POW_MISSILE_1);

		//	Always drop a shield and energy powerup.
		if (Game_mode & GM_MULTI) {
			call_object_create_egg(player, 1, OBJ_POWERUP, POW_SHIELD_BOOST);
			call_object_create_egg(player, 1, OBJ_POWERUP, POW_ENERGY);
		}

//--		//	Drop all the keys.
//--		if (Players[Player_num].flags & PLAYER_FLAGS_BLUE_KEY) {
//--			player->contains_count = 1;
//--			player->contains_type = OBJ_POWERUP;
//--			player->contains_id = POW_KEY_BLUE;
//--			object_create_egg(player);
//--		}
//--		if (Players[Player_num].flags & PLAYER_FLAGS_RED_KEY) {
//--			player->contains_count = 1;
//--			player->contains_type = OBJ_POWERUP;
//--			player->contains_id = POW_KEY_RED;
//--			object_create_egg(player);
//--		}
//--		if (Players[Player_num].flags & PLAYER_FLAGS_GOLD_KEY) {
//--			player->contains_count = 1;
//--			player->contains_type = OBJ_POWERUP;
//--			player->contains_id = POW_KEY_GOLD;
//--			object_create_egg(player);
//--		}
	}
}
